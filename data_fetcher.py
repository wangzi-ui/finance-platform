...
class DataFetcher:
    ...
    def _wait(self):
        ...
        gap = now - last_req
        if gap < MIN_GAP:
            time.sleep(MIN_GAP - gap + random.uniform(0.5, 1.5))
        last_req = time.time()
    ...
    def get_fund_list(self, fund_type='all'):
        ...
        cached = self._cache_get(key, 7200)
        if cached is not None:
            return cached
        try:
            self._wait()
            ...
            if match:
                ...
                for item in items:
                    ...
                    if len(f) >= 14:
                        rows.append({
                            'code': f[0], 'name': f[1],
                            'daily_return': float(f[6] or 0), 'week_return': float(f[7] or 0),
                            'month_return': float(f[8] or 0), 'q3_return': float(f[9] or 0),
                            'half_return': float(f[10] or 0), 'year_return': float(f[11] or 0),
                            'y2_return': float(f[12] or 0), 'y3_return': float(f[13] or 0)
                        })
                df = pd.DataFrame(rows)
                self._cache_set(key, df)
                return df
    ...
    def get_market_index(self):
        ...
        try:
            self._wait()
            codes = 'sh000001,sz399001,sz399006,sh000688,sh000300,sh000905,sh000016,sh000852,sz399303,sz399673,sh931643,bj899050'
    ...
    def get_sector_data(self):
        key = 'sector_data'
        cached = self._cache_get(key, 900)
        if cached is not None:
            return cached
        try:
            self._wait()
            url = "http://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=50&po=1&np=1&fltt=2&invt=2&fid=f3&fs=m:90+t:2&fields=f2,f3,f12,f14"
            resp = requests.get(url, timeout=10)
            data = resp.json()
    ...
    def get_north_flow(self):
        ...
        try:
            self._wait()
            # 用腾讯财经北向资金接口
            url = "http://qt.gtimg.cn/q=ff_bk0551"
            resp = requests.get(url, timeout=10)
            resp.encoding = 'gbk'
            # 东方财富备用
            url2 = "https://push2his.eastmoney.com/api/qt/kamt.kline/get?fields1=f1,f2,f3,f4&fields2=f51,f52,f53,f54&klt=101&lmt=30&secid=1.000001"
            ...
            if data and data.get('data') and data['data'].get('klines'):
                ...
                for item in data['data']['klines']:
                    ...
                    if len(parts) >= 4 and parts[1] != '-':
                        rows.append({
                            '日期': parts[0],
                            '北向净流入(亿)': round(float(parts[1])/10000, 2)
                        })
                if rows:
                    df = pd.DataFrame(rows)
                    self._cache_set(key, df)
    ...
    def get_all_a_stocks(self):
        ...
        try:
            self._wait()
            url = "http://18.push2.eastmoney.com/api/qt/clist/get?pn=1&pz=5000&po=0&np=1&fltt=2&invt=2&fid=f12&fs=m:0+t:6,m:0+t:13,m:0+t:80,m:1+t:2&fields=f12,f14"
            resp = requests.get(url, timeout=15)
            data = resp.json()
            if data and data.get('data') and data['data'].get('diff'):
                rows = [{'code': i.get('f12', ''), 'name': i.get('f14', ''), 'price': 0, 'change_pct': 0, 'volume': 0, 'amount': 0, 'turnover': 0, 'pe': 0} for i in data['data']['diff']]
                df = pd.DataFrame(rows)
                self._cache_set(key, df)
                return df
        except:
